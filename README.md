...
| 变量名 | 说明 | 格式 |
|--------|------|------|
| `ADD` | 本地优选 IP 列表 | `IP:端口#备注名`，多个用 换行 / 空格 / 逗号 分隔 |
| `ADDAPI` | 远程 TXT 优选列表 URL | 多个用 换行 / 空格 / 逗号 分隔 |
| `ADDCSV` | 远程 CSV 优选列表 URL | 多个用 换行 / 空格 / 逗号 分隔，配合 `DLS` 过滤低速 |
| `ADDSUB` | **汇聚订阅**（一个框混写五类内容） | 见下方「ADDSUB 汇聚订阅」章节 |

> **📌 多值分隔符说明（重要）**
>
> Cloudflare 新版 Workers / Pages 的环境变量界面**只能单行输入，不再支持回车换行**。
> 因此 `ADD` / `ADDAPI` / `ADDCSV` / `WL_IP` 这些多值变量已支持以下任意一种分隔方式，**中英文符号不区分**：
>
> | 分隔方式 | 示例 | 适用场景 |
> |---|---|---|
> | 回车换行 | `1.2.3.4:443#A`⏎`5.6.7.8:443#B` | 后台管理面板的多行输入框 |
> | 半角空格 | `1.2.3.4:443#A 5.6.7.8:443#B` | **CF 环境变量界面（推荐）** |
> | 全角空格 | `1.2.3.4:443#A　5.6.7.8:443#B` | CF 环境变量界面 |
> | 英文逗号 `,` | `1.2.3.4:443#A,5.6.7.8:443#B` | CF 环境变量界面 |
> | 中文逗号 `，` | `1.2.3.4:443#A，5.6.7.8:443#B` | CF 环境变量界面 |
> | 顿号 `、` | `1.2.3.4:443#A、5.6.7.8:443#B` | CF 环境变量界面 |
> | Tab | `1.2.3.4:443#A⇥5.6.7.8:443#B` | 从表格粘贴 |
>
> **备注名含空格或逗号不会被误切**：`1.2.3.4:443#美国 洛杉矶 5.6.7.8:443#日本 东京` 会正确识别为 2 个节点，
> 节点名分别是 `美国 洛杉矶` 和 `日本 东京`。判断依据是"`#` 之前的部分是否包含 `.` 或 `:`"，只有像 IP / 域名的片段才会被当作新条目的开头。
>
> ⚠️ **两点注意**
> 1. 分隔时只用**一个**符号即可。若写成 `#A， 5.6.7.8...`（逗号后又加空格），逗号会残留在上一个节点名尾部（变成 `A，`），不影响连接，仅名字略难看。
> 2. 后台管理面板的输入框仍然是多行文本框，用回车换行是最直观的写法，行为与旧版完全一致；`#` 开头的整行仍作为注释被忽略。
...
| 变量名 | 说明 | 格式 |
|--------|------|------|
| `WL_IP` | 管理员 IP 白名单，命中后可免登录访问后台 | 多个 IP 用 空格 / 逗号 / 换行 分隔（中英文符号不区分） |
...
两版 HTTP CONNECT 均支持 Basic Auth 和 Base64 凭证；Snippets 额外支持 UTF-8 凭证及真实 HTTPS CONNECT。Worker 当前只提供明文 HTTP CONNECT，必须填写实际代理端口，不要把 `/https://` 当作 Worker 的 TLS 代理写法。两版均不支持 `/?http=...` 或 `/?https=...` 查询参数；`/https=...` 仅适用于 Snippets。
...
|------|----------|--------------|----------|------|:---:|
| Worker | `/turn://...`、`/turns://...` | `/turn=...`、`/turns=...`，也接受 `/turn=turn://...`、`/turns=turns://...` | `/?turn=...`、`/?turns=...`；追加 `&global=1` 或 `&globalproxy=1` 可改为全局 | 无认证、明文、URL 编码、Base64、UTF-8 | 3478 / 5349 |
...
Snippets 的 `/turn=...`、`/turns=...`、`/?turn=...` 和 `/?turns=...` 不会启用 TURN/TURNS；需要回落或查询参数时请部署 Worker 版。
...
- `ADD`：手动添加优选 IP（格式 `IP:端口#备注`，多个用 换行 / 空格 / 逗号 分隔）
- `ADDAPI`：填入远程 TXT 文件 URL（多个用 换行 / 空格 / 逗号 分隔）
- `ADDCSV`：填入远程 CSV 文件 URL（多个用 换行 / 空格 / 逗号 分隔）
- `DLS`：速度下限筛选（用于 ADDCSV，单位 MB/s）

> 面板输入框用回车换行最直观；若改在 Cloudflare 环境变量界面填写（新版只能单行），请用空格或逗号分隔。详见上文「多值分隔符说明」。
...
- Worker 全局代理：`/turn://用户:密码@地址:3478` 或 `/turns://用户:密码@地址:5349`。
- Worker 直连失败回落：`/turn=用户:密码@地址:3478` 或 `/turns=用户:密码@地址:5349`。
- Worker 查询参数：`/?turn=...`、`/?turns=...`，追加 `&global=1` 可强制全局。
- Snippets：只使用 `/turn://...` 或 `/turns://...` 全局路径。
...
Cloudflare 环境变量界面（只能单行）写法 —— 空格或逗号分隔，效果完全相同：
```
1.2.3.4:443#美国节点1 5.6.7.8:443#日本节点1 cdn.example.com:443#CDN节点
```
```
1.2.3.4:443#美国节点1,5.6.7.8:443#日本节点1,cdn.example.com:443#CDN节点
```

备注名含空格也没问题，会正确识别为 2 个节点：
```
1.2.3.4:443#美国 洛杉矶 5.6.7.8:443#日本 东京
```
...
```
多个 URL 用 换行 / 空格 / 逗号 分隔：
```
https://a.com/ips.txt https://b.com/ips.txt
```
TXT 文件内容格式同 ADD（文件内部仍按行解析）。

3. **ADDCSV**（远程 CSV）：
```
https://example.com/speed.csv
```
多个 URL 同样支持 换行 / 空格 / 逗号 分隔。
